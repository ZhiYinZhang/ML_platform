...
class SamplingOperation(OperationBase):
    ...
    def execute(self):
        ...
        replace = self.get_arg("withReplacement", False)
...
class FillnaOperation(OperationBase):
    ...
    def fillnaDF(self, fillValue, df_in, col):
        label = str(fillValue) + '(' + col + ')'
        ...
        return df_in.fillna(fillValue, col)


    def execute(self):
        df_in: DataFrame = self.elem_dict[self.input[0]]
        print("enter into fillna")
        selectedCols = self.get_arg("selectedColNames")
        print(selectedCols)

        preValue = self.get_arg("preValue")
        fillValue = self.get_arg("fillValue")
        if fillValue is None:
            print('no default fill value, set it to 0')
            fillValue = 0
        cols = df_in.columns
        print(cols)
        if set(cols) > set(selectedCols):
            # df_out = df_in.na.fill({col: fillValue})
            pass
        else:
            self.elem_dict[self.output[0]] = "errorcodexxx"
            print('selected columns not in DataSet')
            return 'selected columns not in DataSet'

        if len(selectedCols) >= 1:
            for col in selectedCols:
                df_in = self.fillnaDF(fillValue, df_in, col)
        else :
            print("no selected columnas")
            return "no selected columnas"
...
class TypeTransformOperation(OperationBase):
    ...
    def execute(self):
        ...
        cols = df_in.columns
        print('current dataset columns as below : ')
        print(cols)
